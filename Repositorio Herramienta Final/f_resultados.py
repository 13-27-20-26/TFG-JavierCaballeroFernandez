...
def plot_results(velocities, resistances, trims, method_names):
    ...
    for i in range(len(resistances)):
        res_kN = np.array(resistances[i]) / 1000  # Convertir a kN
        plt.plot(velocities[i], res_kN, label=method_names[i], linewidth=2.2 )
    plt.xlabel('Velocidad (kn)')
    plt.ylabel('Resistencia (kN)')
    plt.title('Curvas de Resistencia', fontsize=15, fontweight='bold')
    plt.legend()
    plt.grid(True)
    plt.gca().yaxis.set_major_locator(ticker.MultipleLocator(0.5))  # Mostrar ticks cada 0.5 kN
    # Plot trims
    plt.subplot(1, 2, 2)
    for i in range(len(trims)):
        plt.plot(velocities[i], trims[i], label=method_names[i], linewidth=2.2)
    plt.xlabel('Velocidad (kn)')
    plt.ylabel('Trimado (degrees)')
    plt.title('Curvas de Trimado', fontsize=15, fontweight='bold')
    plt.legend()
    plt.grid(True)
...
def plot_lift_curves(velocities_foils, lift_foils, velocities_savitskyfoils, lift_savitskyfoils, velocities_vuelo, lift_vuelo):
    ...
    plt.title('Curvas de Sustentación', fontsize=15, fontweight='bold')
    plt.xlabel('Velocidad (m/s)', fontsize=12, fontweight='bold')
    plt.ylabel('Lift (kN)', fontsize=12, fontweight='bold')
    plt.legend()
    plt.grid(True)
